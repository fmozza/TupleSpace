...
cdef class TupleClient:
    ...
    def __cinit__(self, host: str = "reimann", port: int = 42420):
        ...
        hardcoded_key[:32] = [0x5b, 0x25, 0xc3, 0xc1, 0x00, 0x00, 0x00, 0x00,  # Replace with full key if needed
    ...
    cdef object send_request(self, uint8_t op, PyTuple t, uint64_t id=0):
        ...
        cdef unsigned char nonce[12]  # Declare nonce here
        ...
        ciphertext_len = total_len + 16  # Data + tag
        ciphertext = <unsigned char *>malloc(ciphertext_len)
        if ciphertext == NULL:
            free(plaintext)
            raise MemoryError("Failed to allocate ciphertext buffer")
        
        if encrypt_tuple_encrypt(plaintext, total_len, self.session_key, nonce,
        ...
        memcpy(msg_buffer + 4, nonce, nonce_len)  # Use per-request nonce
